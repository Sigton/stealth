...
class Level01(Level):
    ...
    def __init__(self, player, write_data=False):
        ...
        # Add the level text
        text = text.LevelText("Left/right arrows or A/D to walk", 48, 960)
        self.level_text.add(text)
        text = text.LevelText("Up arrow or W to jump", 48, 990)
        self.level_text.add(text)
        text = text.LevelText("Don't fall!", 615, 600)
        self.level_text.add(text)
        text = text.LevelText("Nearly there...", 975, 600)
        self.level_text.add(text)
        text = text.LevelText("Down we go", 1750, 450)
        self.level_text.add(text)
...
class Level02(Level):
    ...
    def __init__(self, player, write_data=False):
        ...
        # Add the level text
        text = text.LevelText("Watch out for the acid!", 100, 1300)
        self.level_text.add(text)
        text = text.LevelText("Nice!", 850, 1150)
        self.level_text.add(text)
        text = text.LevelText("This is tricky,", 50, 860)
        self.level_text.add(text)
        text = text.LevelText("good luck!", 50, 885)
        self.level_text.add(text)
        text = text.LevelText("Almost there...", 100, 435)
        self.level_text.add(text)
        text = text.LevelText("Use space bar to hack the keypad.", 360, 50)
        self.level_text.add(text)
        text = text.LevelText("Once the keypad is hacked,", 360, 75)
        self.level_text.add(text)
        text = text.LevelText("the door will open.", 360, 100)
        self.level_text.add(text)
        text = text.LevelText("And again", 585, 270)
        self.level_text.add(text)
        text = text.LevelText("Jump!", 800, 370)
        self.level_text.add(text)
        text = text.LevelText("Congrats!", 1150, 700)
        self.level_text.add(text)
        text = text.LevelText("Choose your path", 1300, 625)
        self.level_text.add(text)
...
class Level03(Level):
    ...
    def __init__(self, player, write_data=False):
        ...
        # Add the level text
        text = text.LevelText("Watch out for the guards!", 110, 1075)
        self.level_text.add(text)
        text = text.LevelText("They're searching with torches,", 110, 1100)
        self.level_text.add(text)
        text = text.LevelText("Make sure they don't catch you!", 110, 1125)
        self.level_text.add(text)
        text = text.LevelText("Try to find a way to get past the guard.", 110, 1150)
        self.level_text.add(text)
        text = text.LevelText("Press the jump key", 700, 1300)
        self.level_text.add(text)
        text = text.LevelText("to climb ladders!", 700, 1325)
        self.level_text.add(text)
        text = text.LevelText("Here's another guard.", 500, 900)
        self.level_text.add(text)
        text = text.LevelText("Let go to slide down ladders.", 800, 800)
        self.level_text.add(text)
        text = text.LevelText("These are tricky jumps!", 1120, 1355)
        self.level_text.add(text)
        text = text.LevelText("Press your crouch key", 1635, 1060)
        self.level_text.add(text)
        text = text.LevelText("to slide through", 1635, 1085)
        self.level_text.add(text)
        text = text.LevelText("tight spaces.", 1635, 1110)
        self.level_text.add(text)
        text = text.LevelText("Jump onto", 1200, 875)
        self.level_text.add(text)
        text = text.LevelText("the ladders.", 1200, 900)
        self.level_text.add(text)
        text = text.LevelText("You can also crouch", 920, 350)
        self.level_text.add(text)
        text = text.LevelText("under torches!", 920, 375)
        self.level_text.add(text)
        text = text.LevelText("Good job!", 200, 200)
        self.level_text.add(text)
        text = text.LevelText("Watch out here...", 1275, 425)
        self.level_text.add(text)
...
class Level04(Level):
    ...
    def __init__(self, player, write_data=False):
        ...
        # Add the level text
        text = text.LevelText("Up we go!", 80, 1060)
        self.level_text.add(text)
        text = text.LevelText("Crawl through ventilation shafts", 144, 90)
        self.level_text.add(text)
        text = text.LevelText("Doors aren't", 664, 130)
        self.level_text.add(text)
        text = text.LevelText("always vertical", 664, 155)
        self.level_text.add(text)
        text = text.LevelText("Slide through here!", 588, 476)
        self.level_text.add(text)
        text = text.LevelText("You'll need to hang about on these ladders.", 1200, 1350)
        self.level_text.add(text)
        text = text.LevelText("Here's some complex jumps", 1500, 700)
        self.level_text.add(text)
